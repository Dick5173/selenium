...
import os
#注意HTMLTestRunner的文件名和类名同名
#from package import HTMLTestRunner这里导入的其实只是文件，没导入类
...
if __name__ == '__main__':
    ...
    #3.找到这些测试用例后，执行这些测试用例
    #TextTestRunner()文本的测试用例运行器
    #
    #4.生成测试报告HtmlTestRunner
    #HtmlTestRunner类似于TextTestRunner，都是批量执行测试用例的，区别在于他们执行完所有测试用例的输出结果，一个是text，另一个是html
    #Text会被打印到控制台中，html会单独生成一个文件
    #相比于text，html结构更清晰
    #所以二者选其一，用HtmlTestRunner代替unittest原生的测试用例运行器：TextTestRunner
    #我们需要把生成的html格式的测试报告保存在一个固定的位置方便查看
    #在项目的根节点创建一个文件夹，叫report
