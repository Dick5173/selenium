#这种框架的设计思想，叫做page-object设计模式，是一种高级的框架设计思想
...
# driver.get("http://localhost/index.php?m=user&c=public&a=login")
        ...
        #driver.find_element.,by_classname("login_btn").click()
...
class LoginPage():
    #为网页创建一个构造函数
    #在python中构造函数的固定名字__init__()
    def __init__(self,driver):
        #因为setUp方法中已经创建了一个浏览器，所以这里不需要新建浏览器，直接用setUp创建好的浏览器
...
#声明一个变量，保存元素定位需要的两个参数
    #python的元组，类似于数组
    #这句话的意思是声明了一个数组叫username_input_loc，这个数组中有两个元素分别是By.NAME,"username"
    # 星号表示给find_element这个方法传入的不是一个元组，而是把元组中的每个元素都分别传入find_element()这个方法，作为单独的参数
    ...
    #如果调用方法时不传参，那么使用默认值
    def input_username(self,username='fangyong'):
